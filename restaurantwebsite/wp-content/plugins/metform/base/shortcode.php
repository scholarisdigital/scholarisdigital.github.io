...
class Shortcode
{
	...
	public function render_mf_field($atts){
		...
		if(!is_array($field)){
			return esc_html__("Not entry was found associated with this id", 'metform')."<br>"; // br added if one page have multiple shortcode which is not available
		}
		 
		if(!key_exists($a['field'], $field)){
			return  $a['field'] . esc_html__(" key Not found ", 'metform').'<br>';
		}
